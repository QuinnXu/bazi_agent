  ...
  const { stream } = await runClassicChatStream(
    {
      ...
      guestTrial: input.guestTrial,
      skipUsageTracking: input.skipUsageTracking,
    },
    { signal },
  )
  return stream
...
function buildDirectAnswerInput(
  ...
  const onboardingGuidance: AgentMessage | null = input.guestOnboarding
    ? {
        role: 'system',
        content: buildGuestFirstQaSystemPrompt(input.guestOnboardingQuestion),
      }
    : null
  return {
    ...input,
    pendingConfirmation: null,
    messages: [guidance, ...(onboardingGuidance ? [onboardingGuidance] : []), ...input.messages],
  }
}
...
export async function* runAgentChatEvents(
  ...
  const shouldUseEarlyDirectChat =
    !input.guestOnboarding &&
    (toolDecision?.name === 'agent_direct_chat' || (!toolDecision && shouldDirectChat(input, latest)))
  if (shouldUseEarlyDirectChat) {
    yield traceEvent(trace, {
      step: 1,
      action: 'direct_chat',
      ok: true,
  ...
  let question = input.guestOnboarding && hasDirectChatProfileContext(input)
    ? null
    : planNextQuestion(resolved, sourceText, null, { responseMode: responsePolicy.mode })
  if (question) {
        ...
        for await (const event of runAgentChatEvents(input, deps)) {
          if (opts.onEvent) {
            try {
              ...
              console.error('[agent] event callback failed', callbackError)
            }
          }
          if (event.type === 'delta') {
