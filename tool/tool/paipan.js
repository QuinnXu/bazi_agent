...
function analyzeBazi(year, month, day, hour, is_solar = true, is_female = false, is_leap = false, longitude = 116.38, latitude = 39.90) {
    // 创建 Paipan 实例
    global.p = new paipan();
    
    // 转换输入为日期对象
    const date = new Date(year, month - 1, day, hour, 0, 0);
    const solarOrLunar = is_solar ? 0 : 1;  // 判断是否使用阳历（1为农历）
    const gender = is_female ? 1 : 0;  // 男为0，女为1
    
    // 调用 paipan 的 fatemaps 函数进行排盘
    const result = p.fatemaps(gender, year, month, day, hour, 0, 0, longitude, latitude);
    
    // 提取四柱八字
    const bazi = result.sz.map(s => s).join(' ');  // 四柱八字（年柱 月柱 日柱 时柱）
    
    // 提取大运信息并精简输出
    let dayunStr = '';
    result.dy.forEach((dy, idx) => {
        const startAge = dy.zqage;
        const startYear = dy.syear;
        const endYear = dy.eyear;
        const zfma = dy.zfma + dy.zfmb;  // 大运天干地支
        
        // 格式化大运的年龄和年份输出
        const dayunRange = `${startAge}-${startAge + (endYear - startYear)}岁大运 ${zfma} ${startYear}-${endYear}`;
    ...
    // 输出精简版格式化的八字和大运
    const output = `${is_female ? '女命' : '男命'} 公历: ${year}年${month}月${day}日${hour}时
