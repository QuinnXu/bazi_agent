...
import {
  ...
  buildGuestFirstQaPostRegistrationPrompt,
  isGuestFirstQaFlow,
...
interface GuestTranscriptPayload {
  version: 1
  ...
  postRegistrationFollowUpPending?: boolean
  selectedProfile?: SelectedProfileContext | null
...
function buildGuestOnboardingAnswerPrompt(
  question: string,
  profileName: string = GUEST_FIRST_QA_FLOW.defaultProfileName,
): string {
  return buildGuestFirstQaAnswerPrompt(question, profileName)
}

function buildPostRegistrationFollowUpPrompt(question: string): string {
  return buildGuestFirstQaPostRegistrationPrompt(question)
}

function createGuestOnboardingProfileRequest(question: string): AgentInlineInputRequest {
  ...
  return {
    ...
    submitLabel: GUEST_FIRST_QA_FLOW.copy.profileSubmitLabel,
    resumeIntent: buildGuestOnboardingAnswerPrompt(question, GUEST_FIRST_QA_FLOW.defaultProfileName),
    fields: request.fields.map(field =>
      field.name === 'profileName'
        ? { ...field, value: GUEST_FIRST_QA_FLOW.defaultProfileName }
        : field,
    ),
  }
}

function isGuestOnboardingProfileRequest(request: AgentInlineInputRequest): boolean {
  return isGuestFirstQaRequestId(request.requestId)
}
...
function HomeContent() {
  ...
  const guestSessionIdRef = useRef<string>(createViewOperationId('guest-trial'))
  const landingGuestQuestionRef = useRef<string | null>(null)
  ...
  useEffect(() => {
    ...
    const transcriptMessages: GuestTranscriptMessage[] = messages
      ...
      .map(message => ({
        role: message.role,
        content: sanitizeReplacementChars(message.content),
        createdAt: message.createdAt.toISOString(),
        mode: message.mode || activeChatMode,
        model: message.model ?? null,
        tokensUsed: message.tokensUsed ?? null,
      }))
    ...
    guestTranscriptPayloadRef.current = {
      ...
      postRegistrationFollowUpPending: Boolean(guestOnboardingQuestion && guestFinalAnswerUsed),
      selectedProfile,
      baziAnalysisResult,
    }
  ...
  useEffect(() => {
    ...
    const migrate = async () => {
      ...
      let { data: session, error: sessionError } = await supabase
        ...
        .insert({
          ...
          title,
          mode,
          summary: payload?.sessionSummary || payload?.featureContext?.summary || null,
        } as any)
        .select()
        .single()
      if (sessionError && isMissingModeColumn(sessionError)) {
      ...
      if (sessionError || !session) throw sessionError || new Error('迁移试用会话失败')

      const rows = transcriptMessages.map(message => ({
        session_id: session.id,
        role: message.role,
        content: sanitizeReplacementChars(message.content),
        mode: message.mode === 'classic' ? 'classic' : 'agent',
        model: message.model ?? null,
        tokens_used: typeof message.tokensUsed === 'number'
          ? message.tokensUsed
          : estimateTokensForText(message.content),
        created_at: message.createdAt,
      }))

      let { error: messagesError } = await supabase
        .from('chat_messages')
        .insert(rows as any)
      if (messagesError && (isMissingModeColumn(messagesError) || isMissingMessageMetadataColumn(messagesError))) {
        const retryRows = rows.map(message => ({
          session_id: message.session_id,
          role: message.role,
          content: message.content,
          created_at: message.created_at,
          ...(isMissingModeColumn(messagesError) ? {} : { mode: message.mode }),
        }))
        const retry = await supabase
          .from('chat_messages')
          .insert(retryRows as any)
        messagesError = retry.error
      }
      if (messagesError) throw messagesError

      const restoredProfile = payload?.selectedProfile && payload.selectedProfile.name
        ? payload.selectedProfile
        : null
      const postRegistrationQuestion =
        payload?.postRegistrationFollowUpPending && payload.guestOnboardingQuestion
          ? payload.guestOnboardingQuestion.trim()
          : ''

      window.localStorage.removeItem(GUEST_TRANSCRIPT_STORAGE_KEY)
      window.localStorage.removeItem(GUEST_TRIAL_USED_STORAGE_KEY)
      setGuestFinalAnswerUsed(false)
      setGuestOnboardingQuestion(null)
      landingGuestQuestionRef.current = null
      ...
      if (restoredProfile) {
        setSelectedProfile(restoredProfile)
        setSelectedProfileId(restoredProfile.id || null)
        setAgentParticipants(prev => mergeProfileContexts([...prev, restoredProfile]))
      }
      if (payload?.baziAnalysisResult) {
        setBaziAnalysisResult(payload.baziAnalysisResult)
      } else if (restoredProfile?.baziText) {
        setBaziAnalysisResult(restoredProfile.baziText)
      }
      if (postRegistrationQuestion) {
        setPendingPostRegistrationFollowUp({
          sessionId: session.id,
          question: postRegistrationQuestion,
        })
      }
      refreshSessionList()
    }
    ...
    void migrate().catch(error => {
      console.error('[guest-trial] 迁移试用记录失败:', error)
    }).finally(() => {
      guestTranscriptMigrationRef.current = false
    })
  }, [refreshSessionList, supabase, user])
  ...
  const handleChatModeChange = useCallback((mode: ChatMode) => {
    ...
    setMentionOpen(false)
    setComposerModeOpen(false)
    if (currentSessionId && currentSessionId !== 'new' && user && !isTemporaryChat) {
      supabase
        .from('chat_sessions')
        .update({ mode } as any)
        .eq('id', currentSessionId)
  ...
  const handleTemporaryChatToggle = useCallback(() => {
    ...
    const nextEnabled = !isTemporaryChat
    const localSessionId = nextEnabled ? createViewOperationId('temporary') : null
    temporarySessionIdRef.current = localSessionId
    setIsTemporaryChat(nextEnabled)
    beginSessionView(localSessionId)
    selectedSessionIdRef.current = localSessionId
    setCurrentSessionId(null)
    setCurrentSessionMode(activeChatMode)
    setMessages([])
    setFeatureContext(null)
    setSessionSummary(null)
    setAgentPendingConfirmation(null)
  ...
  }, [currentSessionId, fetchQuota, isLoading, isTemporaryChat, pollDurableRun, requestFollowUpSuggestions, setStreamingMessageId, user])
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    const viewSeq = sessionViewSeqRef.current
    activeOperationIdRef.current = operationId
    let sessionId: string | null = shouldPersistChat && currentSessionId && currentSessionId !== 'new'
      ? currentSessionId
      : null
    if (!shouldPersistChat) {
      sessionId = isGuestTrialRequest
        ? guestSessionIdRef.current
        : temporarySessionIdRef.current || createViewOperationId('temporary')
      if (!isGuestTrialRequest && isTemporaryChat) {
        temporarySessionIdRef.current = sessionId
      }
      selectedSessionIdRef.current = sessionId
    }
    ...
    const runKind: MessageRunKind = effectiveChatMode === 'agent' ? 'agent' : 'classic'
    const assistantMessage: Message = {
      id: createBubuMessageId('assistant'),
      role: 'assistant',
      content: '',
    ...
    try {
      ...
      if (effectiveChatMode === 'agent') {
        requestData.complexity = isGuestTrialRequest
          ? GUEST_FIRST_QA_FLOW.complexity
          : complexityOverride || agentComplexity
        if (sessionSummary) {
        ...
        if (isGuestOnboardingRequest) {
          const onboardingQuestion =
            submitEvent.__guestOnboardingQuestion?.trim() ||
            guestOnboardingQuestion ||
            landingGuestQuestionRef.current
          requestData.guestOnboarding = true
          requestData.guestOnboardingQuestion = onboardingQuestion || submittedText
        }
      }
      if (effectiveChatMode === 'agent' && requestSelectedProfile) {
      ...
      const response = await fetch(effectiveChatMode === 'agent' ? '/api/agent-chat' : '/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      const event = {
        ...
        __agentComplexityOverride: GUEST_FIRST_QA_FLOW.complexity as AgentComplexityMode,
        __pendingConfirmationOverride: null,
      } as React.FormEvent & {
        __contentOverride: string
        __preserveCurrentProfile: boolean
        __agentReportPreferenceOverride: AgentReportPreference
        __agentComplexityOverride: AgentComplexityMode
        __pendingConfirmationOverride: AgentPendingConfirmation | null
      }
      handleSubmit(event)
    }, 350)
    ...
    try {
      ...
      if (request.kind === 'bazi_profile' || request.kind === 'profile_required') {
        ...
        const event = {
          ...
          __preserveCurrentProfile: boolean
          __guestOnboarding?: boolean
          __guestOnboardingQuestion?: string
          __agentReportPreferenceOverride?: AgentReportPreference | null
          __agentComplexityOverride?: AgentComplexityMode | null
          __pendingConfirmationOverride?: AgentPendingConfirmation | null
        }
  ...
  }, [agentParticipants, agentPendingConfirmation, createAndSaveBaziProfile, guestOnboardingQuestion, handleSubmit, selectedProfile])
              ...
              <div className="flex items-end gap-2">
                ...
                {user && activeChatMode !== 'agent' && (
                  <button
                    ...
                    title={isTemporaryChat ? '临时聊天已开启：不会保存记录' : '开启临时聊天：不保存当前对话'}
                  >
