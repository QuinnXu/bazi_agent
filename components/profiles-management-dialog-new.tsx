      ...
      if (editingProfile) {
        ...
        const { error } = await supabase
          ...
          .update({
            ...
            is_solar: data.isSolar,
            is_female: data.isFemale,
            longitude: parseFloat(data.longitude),
            latitude: parseFloat(data.latitude),
            bazi_result: result.baziResult,
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-neutral-600">
                          {profile.year}年{profile.month}月{profile.day}日 {profile.hour}:{profile.minute.toString().padStart(2, '0')}
                        </p>
                        <p className="text-xs text-neutral-500 mt-1">
                          {profile.is_solar ? '阳历' : '阴历'} · {profile.is_female ? '女' : '男'}
                        </p>
                      </div>
      ...
      {dialogStep === 'bazi' && (
        <BaziDialog
          ...
          initialData={editingProfile ? {
            year: editingProfile.year.toString(),
            month: editingProfile.month.toString(),
            day: editingProfile.day.toString(),
            hour: editingProfile.hour.toString(),
            minute: editingProfile.minute.toString(),
            isSolar: editingProfile.is_solar,
            isFemale: editingProfile.is_female,
            longitude: editingProfile.longitude.toString(),
            latitude: editingProfile.latitude.toString(),
          } : undefined}
        />
      )}
    </>
